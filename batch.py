...
os.chdir("E:\Brooklyn Nine-Nine\Season 8")


def _get_args():
    parser = argparse.ArgumentParser(
        description="A useful tool that \
    ...
    parser.add_argument(
        "-e", "--encoding", help="charset of input ASS file (default: auto detect)"
    )
    parser.add_argument(
        "-t",
        "--translate-to",
    ...
    parser.add_argument(
        ...
        "--opencc",
        dest="opencc_config",
        help="Use OpenCC to convert Simplified/Traditional Chinese " "(need pyopencc)'",
    )
    parser.add_argument(
        "-n", "--no-effact", action="store_true", help="ignore all effact text"
    )
    parser.add_argument(
        "-l",
        "--only-first-line",
    ...
    parser.add_argument(
        "-s", "--srt-encoding", help="charset of output SRT file (default: same as ASS)"
    )
    parser.add_argument(
        "-o",
        "--output-dir",
...
def _check_chardet():
    ...
    try:
        ...
        print(
            'Error: module "chardet" not found.\nPlease install it '
...
def _detect_charset(file):
    ...
    if c["confidence"] < 0.3:
        print(
            "Error: unknown file encoding, " + "please specify one by -e.",
            file=sys.stderr,
        )
        sys.exit(1)
    elif c["confidence"] < 0.6:
        print(
            "Warning: uncertain file encoding, " + "suggest specify one by -e.",
            file=sys.stderr,
        )
...
def _convert_files(files, args):
    ...
    for in_path, out_path in _combine_output_file_path(files, args.output_dir):
        print(
            "\t({:02d}/{:02d}) is converting... ".format(done + fail + ignore + 1, sum),
            end="",
        )
        ...
        try:
            with open(in_path, "rb") as in_file:
                ...
                out_str = asstosrt.convert(
                    in_codec.streamreader(in_file),
                    args.translator,
                    args.no_effact,
    ...
    print(
        "All done:\n\t{} success, {} ignore, {} fail.".format(
            done, ignore, sum - done - ignore
        )
    )
...
def main():
    ...
    try:
        ...
        print(
            "Error: {}\nPlease install it or disable " "translation.".format(e.message)
        )
        sys.exit(1)
    ...
    if not files:
        print(
            "ASS/SSA file no found.\nTry --help for more information", file=sys.stderr
        )
        sys.exit(1)
